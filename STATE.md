...
**Fix-up pass — PR #7 squash-merge `ae3f7e1` on `main`:**
- 52 lorem ipsum zaps via Latin-word density heuristic (covers `settings.editor`, `settings.description_text`, `ekit_accordion_items[*].acc_content`).
- `home.json`: `_wp_img: "hero"` on first hero container; `{{services._item.description}}` on repeat-card text-editor.
- `about.json`: `_wp_img: "about"` on first image widget.
- `services.json`: same `{{services._item.description}}`; removed 8 stray `_wp_repeat: services` leaf-widget markers (left one valid container with 2 children).
- `header.json`: injected `{{business_name}}` heading widget alongside logo.
- `contact.json`: 4 icon-box `description_text` fields fixed with `{{contact.*}}` tokens (Head Office → address, Let's Talk → phone, Email Support → email, Office Hour → static hours).
- Deploy payload normalized `industry: "media-personality" → "other"` so engine stock-fallback chain resolves to `other.generic` instead of an unknown trade.

**Redeploy on `webprinter-test.hostingersite.com` via residential curl (Hostinger DO-ASN bot-wall still active on n8n Deploy node — Issue #5 recurs on test site):**
- Engine HTTP 200 in 12.2s, version `5.3-candidate`, all 7 surfaces updated, `image_ids: {hero:155, about:156, service:169, logo:0, gallery_0-3:158-161}`. `service` slot now falls back to `other/generic/other-generic-02.jpg` (was `general/generic/...` with the bad industry value).

**QA chain re-run on the redeployed site: `qa_score: 52, qa_verdict: FAIL` (Δ +18 vs prior 34/FAIL on the same template, same source, same engine).** 13 issues vs 16, with the lorem-ipsum-on-mobile, services-page z-index overlap, header business-name missing, and template-placeholder-brand vision flags all cleared. Remaining critical/high (4 critical, 5 high) split into three layers:
- **Template body authoring left to do**: hero CTA missing (Modora hero button still hardcoded — needs `{{cta.primary_text}}` tokenization), services page has a large empty black band immediately below header (the section needs an image or `_wp_if`), home desktop has a gray gap below hero (section padding), footer renders but isn't visible in viewport (likely contrast or scroll height).
- **Payload-side (real data limits)**: `bethebesthome.com` doesn't publicly expose phone/email/address → `contact.*` all empty → vision flags "no contact info" / "no click-to-call" / "no footer info" (this is accurate, not a template bug). No testimonials in source → "no social proof" flag.
- **CSS / responsive**: mobile overflow + horizontal bleed at 390px, typography hierarchy on mobile, mobile image stacking — all kit `custom_css` work.

**TEA-760 SnapRender decision update**: still **CANCEL** — vision continues to read the rendered site accurately (named specific layout defects across both runs, scoring them differently as content quality improved). Firecrawl screenshots are not the bottleneck on this template. The screenshot swap doesn't address any of the 13 remaining issues.

---

**Last STATE entry before TEA-792 Wake 1 (Wake 5, TEA-756 close — Path 1, accept v3 as v0.1 cold-start baseline).** Cloned gold workflow `A6naBMqLH3eRDzjx` → new workflow `wp-pilot webprinter` (id `tLIE3cHlwWF97FRD`) with webhook path `wp-pilot-webprinter`, deploy URL pointed at `webprinter-test.hostingersite.com`, Basic auth from `WP_TEST_USER:WP_TEST_PASSWORD`, validTemplates extended with `'modora'`. Gold workflow untouched. Staged Modora Elementor kit (TEA-760 attachment) as `templates/modora/{home,about,services,quote,contact,header,footer}.json + kit.json` via branch `tea792-modora` → PR #6 squash-merge `f453cc1`. Scraped `bethebesthome.com` via the cloned pipeline (n8n exec 1142): business_name=Kathryn Emery, industry=media-personality, 5 services, hero from source. Deploy via residential-curl bypass of Hostinger bot-wall (same DigitalOcean ASN issue, Issue #5 recurs on webprinter-test). Engine returned **HTTP 200 in ~4s** with engine shape `{success:true, version:"5.3-candidate", template:"modora", company:"Kathryn Emery", updated:{home,about,services,quote,contact,header:"updated",footer:"updated"}, image_ids:{hero:155, about:156, service:157, logo:0, gallery_0-3:158-161}}`. **QA chain exec on `webprinter-test.hostingersite.com` returned qa_score=34, qa_verdict=FAIL** — regression from ainexa baseline 54-58. 16 issues, of which 10 critical/high are layout/template-side (lorem ipsum bleed-through on mobile, services-page z-index collisions, empty white box, gray gap below hero, no hero CTA, business name missing from header, no contact info / no footer rendered, mobile no tappable buttons). Vision read the rendered site accurately enough to name specific layout defects → **Firecrawl screenshots are not the Vision bottleneck; SnapRender swap (TEA-760) does not address any actual blocker → recommend CANCEL TEA-760**. Two new infra notes captured below: (a) `webprinter-test.hostingersite.com` runs engine **version `5.3-candidate`** (not `5.2`), and design_tokens override on this build trips Elementor `Core\Settings\Page\Manager::ajax_before_save_settings()` Access Denied 500 — workaround was to omit `design_tokens`+`design_profile` and pass `brand_mode:"template"`; (b) Hostinger DigitalOcean-ASN bot-wall (Issue #5) is also live on `webprinter-test.hostingersite.com`, not just `getinstabid.pro`.
